...
|       Database        |        SQL  |     NoSQL  |  
| --------------------- |-------------|------------| 
|     Data uniformness  | Uniform data. \n Best visualized as a set of tables. \n Each table has rows, with each row representing an entity of interest. \n Each row is described through columns. One row cannot be nested inside another. | Non-uniform data. \n NoSQL databases recognize that often, it is common to operate on data in units that have a more complex structure than a set of rows. This is particularly useful in dealing with nonuniform data and custom fields. NoSQL data model can be put into four categories: key-value, document, column-family and graph. |
